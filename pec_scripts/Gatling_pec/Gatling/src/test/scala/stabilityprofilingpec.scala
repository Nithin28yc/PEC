...
class stabilityprofilingpec extends Simulation {
  ...
  private val uri1 = "https://course-api.com"
  ...
  private val scn = scenario("stabilityprofilingpec")
    .feed(csvfeeder)
    .exec(
      http("request_0")
        .get("/pec/")
    ...
    .exec(
      http("request_3")
        .post("/auth/realms/devopsplatform/login-actions/authenticate?session_code=jAcZO50p9j-7skMbzOi9iI0b7f7GLljBBQz5lOkiYKY&execution=e3a7689d-a5c1-446c-88dd-7fd3b960a3bd&client_id=devops&tab_id=4ZQROqp8ze0")
        .headers(headers_3)
    ...
    .exec(
      http("request_6")
        .get(uri1 + "/javascript-store-products")
        .headers(headers_6)
    )
    .pause(1)
    .exec(
    ...
    .exec(
      http("request_10")
        .get("/pec/product.html?id=recZEougL5bbY4AEx")
        .headers(headers_7)
        .resources(
          http("request_11")
            .get("/pec/images/logo-black.svg")
            .headers(headers_2),
          http("request_12")
            .get(uri1 + "/javascript-store-single-product?id=recZEougL5bbY4AEx")
            .headers(headers_6)
        )
    )
    .pause(9)
